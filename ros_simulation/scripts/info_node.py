...
class InfoNode:
    def __init__(self):
        ...
        # Desired position
        self.des_x = rospy.get_param("~des_x",0)
        self.des_y = rospy.get_param("~des_y",0)
        
        # Frequency of the node in Hz
        self.frequency = rospy.get_param("~publish_frequency",1)
        
        # Time of the last print
        self.old_time = rospy.get_rostime().to_sec() * 1000
    
    def callback(self, msg):
        # Get the current time in milliseconds
        current_time = rospy.get_rostime().to_sec() * 1000
